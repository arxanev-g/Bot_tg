...
void ClearOffsetBetweenTests() {
  // чищу оффсет после каждого теста, потому что если запускать их по
  очереди, то после 3го теста
               // оффсет из 3го передаётся 4му, и он падает, как я понимаю если
               прописать для всех сценариев
                   // последовательные update_id, всё тоже будет хорошо
                   // работать, но два
                       запуска test_telegram
                           // подряд тоже упадут, ибо нужно будет чистить между
                           // ними файл
